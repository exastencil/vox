...
pub const WorldRegistry = struct {
    allocator: std.mem.Allocator,
    by_key: std.StringHashMap(WorldDef),

    pub fn init(allocator: std.mem.Allocator) WorldRegistry {
        return .{ .allocator = allocator, .by_key = std.StringHashMap(WorldDef).init(allocator) };
    }

    pub fn deinit(self: *WorldRegistry) void {
    ...
    pub fn addWorld(self: *WorldRegistry, key: []const u8, display_name: []const u8, section_count_y: u16) !void {
    ...
    pub fn get(self: *const WorldRegistry, key: []const u8) ?WorldDef {
